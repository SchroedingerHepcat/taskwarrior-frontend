  ...
  Future<List<TaskItem>> queryTasks(TaskQuery query) async {
    final response = await _client.post(
      ...
      body: jsonEncode(<String, dynamic>{
        ...
        'no_project': query.noProject,
        'required_tag': query.requiredTag,
        'no_tags': query.noTags,
        'due_after': query.dueAfter?.toUtc().toIso8601String(),
        'due_before': query.dueBefore?.toUtc().toIso8601String(),
        'scheduled_after': query.scheduledAfter?.toUtc().toIso8601String(),
        'scheduled_before': query.scheduledBefore?.toUtc().toIso8601String(),
        'wait_after': query.waitAfter?.toUtc().toIso8601String(),
        'wait_before': query.waitBefore?.toUtc().toIso8601String(),
